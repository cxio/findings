...
因为P2P节点之间无需信任，所以采用自签名证书创建安全连接，由节点地址对应的 SPKI（Subject Public Key Info）指纹验证公钥的匹配性。
...
当新的连接创建后，双方会彼此分享自己候选池中的一部分节点信息。分享的节点在合并前会做必要的测试（可连接性、证书SPKI指纹是否匹配等）。
...
攻击者可能通过收集Findings节点的IP地址，然后阻断或破坏它们的连接与传输。虽然Findings网络是P2P的，但如果大量节点被阻断，这依然不是一件好事。

因此设计支持用户可配置注入大平台/网站的真实IP，用于混淆Findings节点的有效地址。

同时，候选池在添加分享IP时，应保留模糊性，避免干扰IP被全部清除。可以参考如下实现：

- 创建一个临时集合，内容为候选池成员副本和对端分享的IP集。
- 创建一个空的候选池大小的中转池。
- 从临时集合中随机抽取IP测试，添加到中转池，直到中转池大小达到候选池大小 `1/2 ~ 2/3` 的数量。
- 剩余的IP直接添加到中转池，直到中转池满。
- 用中转池替换候选池。

> **背景：**
> 应用客户端不应当期待自己得到的Findings节点是完全有效的，实际上客户端也会先尝试连接，无效的节点会丢弃。
...
接受应用客户端的请求，向指定域名/IP转发数据，帮助客户端隐藏真实地址（解耦IP关联）。

- 远端的接受数据的节点应当有会话恢复的能力，因为数据来源的桥节点通常是随机的，也可能随时断开。
- 客户端和远端节点之间可通过预先配置的共享密钥加密数据，桥节点不需要解密。


#### 认证

- 客户端向桥节点请求定向桥，附带信息包含：域名或IP:端口、SPKI指纹、shortID、会话标识（SN）。
- 桥节点连接目标地址，用 SPKI 指纹验证证书，创建安全连接，传递 shortID 和会话标识。
- 远端节点用 shortID 检索预设的共享密钥，加密会话标识和一个新的会话密钥、时间戳回传。
- 桥节点收到远端节点的响应后，传递给客户端，客户端用共享密钥解密，验证会话标识并获得会话密钥。
- 客户端和远端节点之间用新的会话密钥交互，数据内容与桥节点无关。


#### 会话恢复

- 客户端向桥节点请求定向桥，附带信息包含：域名或IP:端口、SPKI指纹、SessID、加密数据。
- 桥节点连接目标地址，用 SPKI 指纹验证证书，创建安全连接，传递 SessID 和已加密数据。
- 远端节点用 SessID 检索预设的共享密钥，解密数据，继续交互……

> **提示：**
> SessID 和 shortID 都可以引用会话密钥，可以嵌入标识以区分。


#### 附注

定向桥是 Findings 节点的一个扩展功能，它会消耗流量和带宽。节点可以选择关闭它（默认开启）。

之所以默认开启，是想营造一个更大众化的保护网络，毕竟越多人启用，分摊的负载就越低。所谓人人为我，我为人人。

> **注：**
> 定向桥类似于Cloudflare的橙云代理，但更简单和自由。
