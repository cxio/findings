...
服务端运行一个由自签名支持的探测服务器（quic.Listen），已知其自签名证书的 SPKI、服务器地址（IP:Port，公网或 `FullC`）。
...
基于此连接创建一个到相同服务地址的 QUIC 拨号（quic.Conn）连接，使用服务器 SPKI 验证核实证书。
...
与此同时，客户端在旧链路（纯UDP）上会发送一个 PING 包，以刷新 NAT 映射，同时也重置下一轮探测的计时（回到零点）。
...
> 粗测和精测是各自独立的，精测需要重新走一遍流程：从初始的客户端 QUIC 拨号开始（创建NAT映射）。
